...
## App
Two modes available:
- **Random Transaction** — picks a real transaction from the dataset and predicts whether it's fraud or legit, with the actual label shown for comparison. A "Pick Random Fraud" button is also available for guaranteed fraud examples.
- **Custom Input** — enter a transaction amount and hour of day for a quick prediction. V1–V28 features default to neutral (zero-mean) values since they are PCA-anonymized bank features.
